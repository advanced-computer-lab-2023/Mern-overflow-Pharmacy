...
export default function PatientViewCart(props) {
    ...
    const handleDelete = (medName) => {
        ...
        axios.delete(`http://localhost:8001/cart/${userId}/${medName}`)
            .then((response) => {
                fetchTableData();
    ...
    const handleChangeAmount = (medName, newAmount) => {
        setLoadingChange(true);
        axios.post(`http://localhost:8001/cart/${userId}/changeAmount`, { medName, newAmount })
            .then((response) => {
                setLoadingChange(false);
                fetchTableData();
            })
                        ...
                        {meds.length > 0 ? (
                            <>
                                <Table sx={{ mt: "20px" }}>
                                    <TableHead>
                                        <TableRow>
                                            <TableCell key="name" sx={{ textAlign: "center", fontWeight: "bold" }}>
                                                Medicine Name
                                            </TableCell>
                                            <TableCell key="price" sx={{ textAlign: "center", fontWeight: "bold" }}>
                                                Medicine Price
                                            </TableCell>
                                            <TableCell key="quantity" sx={{ textAlign: "center", fontWeight: "bold" }}>
                                                Quantity in Cart
                                            </TableCell>
                                            <TableCell key="quantity" sx={{ textAlign: "center", fontWeight: "bold" }}>
                                                Total Price
                                            </TableCell>
                                            <TableCell key="action" sx={{ textAlign: "center", fontWeight: "bold" }}>
                                                Remove from Cart{" "}
                                            </TableCell>
                                        </TableRow>
                                    </TableHead>
                                        ...
                                        {meds.map((med) => (
                                            <TableRow key={med.medName}>
                                                <TableCell sx={{ textAlign: "center" }}>
                                                    {" "}
                                                    {capitalize(med.medName)}{" "}
                                                </TableCell>
                                                <TableCell sx={{ textAlign: "center" }}> EGP {med.medPrice} </TableCell>
                                                <TableCell sx={{ textAlign: "center" }}>
                                                    <IconButton
                                                        onClick={() =>
                                                            handleChangeAmount(med.medName, med.medQuantity - 1)
                                                        }
                                                    >
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        >
                                            {" "}
                                            Back to medicines <MedicationIcon sx={{ ml: "15px" }} />{" "}
                                        </Button>
                                        <Button variant="contained" component={Link} to="/patient/checkout">
                                            {" "}
                                            Checkout <PointOfSaleIcon sx={{ ml: "15px" }} />{" "}
                                        </Button>
                                    </div>
                                </Container>
                            </>
                            ...
                            <>
                                ...
                                <Typography variant="h6">
                                    Check out our available medicines and start adding to your cart now!
                                </Typography>
